...
user_info AS (
    SELECT
        ...
        PLAN_TYPE,
        IS_ACTIVE
    FROM DIESELPJ_TEST.DBT_HANDSON.USERS
)
...
user_attributes AS (
    ...
    FROM filtered_events fe
    INNER JOIN DIESELPJ_TEST.DBT_HANDSON.USERS u ON fe.USER_ID = u.USER_ID
    GROUP BY DATE(fe.EVENT_TIMESTAMP), u.COUNTRY, u.PLAN_TYPE
)
