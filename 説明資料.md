...
CREATE SCHEMA DIESELPJ_TEST.intermediate;
...
WITH source_events AS (
    SELECT
        ...
        EVENT_TIMESTAMP, DEVICE_TYPE, COUNTRY
...
| 1コマ目（SQL） | dbt |
|---------------|-----|
| 1つのSQLファイルに複数CTEを詰め込む | **各CTEが独立したSQLファイル** |
| `WITH daily_events AS (...)` | `stg_events_beginner.sql` |
...
WITH source_users AS (
    SELECT USER_ID, SIGNUP_DATE, COUNTRY, PLAN_TYPE, IS_ACTIVE
...
**ポイント**：
- `{{ source('DIESELPJ_TEST', 'RAW_EVENTS') }}` でソーステーブルを参照（dbtが依存関係を認識）
- `{{ config(...) }}` でマテリアライズ方法を指定
- Step Aで学んだ SELECT, WHERE をそのまま活用

---

### Step H：Martsモデル -「壁4」を突破（10分）

**目的**：SP代替としてのdbtモデルを体験

**解決する壁**：
- 壁4（SP複雑化）→ 各モデルが独立SQLファイル、`ref()`で依存管理
...
joined AS (
    SELECT
        E.EVENT_DATE, U.COUNTRY,
        E.EVENT_ID, E.USER_ID, E.EVENT_TYPE
    FROM events E
...
- `ref()` はモデル名を指定するだけで、dbtが実行順序を自動決定
- SPでは手動で「まずstg_eventsを実行、次にdaily_summaryを実行」と管理していた
- dbtでは `ref()` があれば自動的に正しい順序で実行される
